...
import type { BibleCatalogBook } from "./bible-navigation";
import {
  enrichTranslation,
...
export function getBibleNavigationBooks(): BibleCatalogBook[] {
  const db = getBibleDb();
  try {
    const rows = db.prepare(`
      SELECT b.code, b.name_fr, b.testament, v.chapter, MAX(v.verse) AS last_verse
      FROM books b JOIN verses v ON v.book = b.code
      GROUP BY b.code, b.order_num, v.chapter
      ORDER BY b.order_num, v.chapter
    `).all() as Array<{ code: string; name_fr: string; testament: 'OT' | 'NT'; chapter: number; last_verse: number }>;
    const books = new Map<string, BibleCatalogBook>();
    for (const row of rows) {
      let book = books.get(row.code);
      if (!book) {
        book = { code: row.code, name: row.name_fr, testament: row.testament, chapters: [] };
